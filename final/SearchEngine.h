	...
	SCORE negaScout(const BOARD &B, SCORE alpha, SCORE beta, MOV &BestMove, const int depth){
		...
		if(record.val != nullptr){
			...
			if(depth != 0){
				...
				// exact
				if(*record.flag == 0){
					// ≤zΩ◊§W°G∞≤≥] A node ©≥§U¶≥§@®B∑|®´®Ï A node, ®∫≤ƒ§@¶∏∑j™∫Æ…≠‘ §U≠±™∫ A node ∞O¶b hash ™∫≠»∑|≥Qª\±º
					// ©“•H≥Ã´·Ød§U®”™∫ hash ≠»∑|¨O root A node
					// ¶]¶π°A≤ƒ§G¶∏∑j™∫Æ…≠‘°A¶p™G©≥§U≥o§@¶Ê®œ•Œ <= ™∫∏‹°A∑|≥y¶®©≥§U™∫ A node ®œ•Œ™∫¨O ≤ƒ§@¶∏∑j¥MÆ… root A node ™∫≠»
					// ©“•H≤ƒ§G¶∏∑j¥M§~∑|≥y¶®§£¶Pµ≤™G°I°I°I(assert(*record.val == m) ∑|ø˘™∫≠Ï¶])
					// πÍ¥˙ 70 ¥X≥ıµo≤{¶p™G•u•Œ≥o≠”¿≥∏”¥N§£∑|ºQ§F°I°C
					if((this->cutOffDepth - depth) == *record.depth){
						// •i•H™Ω±µ¶^∂«
						return *record.val; // ≥o§@®B•iØ‡∑|≥y¶®≤`´◊§£¶P ¶”¶≥§£¶P™∫≠»°H
					}
				}
				// lower bound(ß‚ alpha ©‘∞™§@¬I)
				// ≤zΩ◊§W°G≤ƒ§@¶∏ A node ©≥§U©π§U∑j™∫Æ…≠‘°A•iØ‡©≥§U™∫≠´Ω∆ΩL≠± A node ¶≥≥Q cache ¶Ì(¶˝¨O¨O§Ò∏˚≤Lºh™∫)°A¶]¶π root A node ∑j™∫Æ…≠‘, ∑|®œ•Œ®Ï≤Lºh cache ¶Ì™∫≠», ¶”≥o≠” cache ™∫≠»•iØ‡∑|≥y¶®§@∂}©l alpha ¥N≥]±o§Ò∏˚∞™°]¶PÆ… ≥o≠” cache ≠»•iØ‡¨O±q≤ƒ 4 ≠” branch ß‰®Ï™∫°A¶˝§@∂}©l¥N≥]©w¨∞ alpha •iØ‡∑|≥y¶®´e≠±™∫ branch ºQ•X§£¶PºQ•X§£¶P™∫≠»
				// ¶˝≤ƒ§G¶∏ A node ∑j™∫Æ…≠‘°A¶A§@¶∏∏I®Ï©≥§U™∫≠´Ω∆ΩL≠± A node Æ…°A¶]¨∞ A node §w∏g≥Q≤`§@ºh™∫≠»¥´±º§F°A¶]¶π•Œ§£®Ï cache ™∫≠»§F°A©“•H®œ•Œ™∫ alpha ¨O•L™∫ parent ∂«®”™∫
				// µ≤Ω◊°G¥`¿ÙΩL≠±Æ…•Œ TTB ≠n´‹§p§ﬂ°I°I°I°I
				// else if(*record.flag == 1){
				// 	// ≥o√‰§]≠nßÔ¶® ==°A•H¡◊ßK negaScout §§¶≥ loop
				// 	if((this->cutOffDepth - depth) == *record.depth){
				...
				// }
				// // ¶nπ≥¶p™G alpha, beta ¶p™G bound ®S¶≥Ωƒ®Ï°A¿≥∏”≠n•Œ¶^≠Ï®”™∫ alpha beta, §£µM¶nπ≥(§£¨O´‹ΩT©w)∑|≥y¶® assert(m == *record.val) ºQ±º?
				// else{
				// 	// ¡Ÿ≠Ï alpha, beta(¶p™G§ß´e¶≥ßÔ®Ï)
				// 	// ∏gπLπÍ¥˙(100≥ı)µo≤{≥oºÀ•[¥N§£∑| assert(*record.val == m) ºQ±º
				// 	// ß⁄ƒ±±o¨O•iØ‡¨O¶]¨∞¶p™GßAß‚ alpha, beta bound ≈‹Ø∂Æ…°A•·§U•h Scout ©Œ negaMax ∑j™∫Æ…≠‘Æ≥®Ï™∫≠»•iØ‡∑|§£§@ºÀ(¶]¨∞¥£®Ï break ™∫Æ…∂°¬I∑|§£§@ºÀ)
				// 	// ©“•H≥y¶® assert(*record.val == m) ∑|ºQ±º°A¶]¨∞ßA•iØ‡Æ≥®Ï™∫ Scout, negaMax ≠»∑|§£¶P
		...
		for(int i = 0; i < lst.num; i++){
			...
			if(t > m){
				...
				if(depth == 0){
					...
					#ifdef ATTACKMOVE
					// FIXME: BUG ¶]¨∞¶p™G¥`¿ÙΩL≠±™∫∏‹, ¶]¨∞ transposition table caching ™∫√ˆ´Y, ∑|≈˝≥o§@®B attack move •[ßπ™∫≠»≥Q¶s¶b table ∏Ã≠±≥y¶®ª~ßP
			...
			n = MAX(alpha, m) + 0.1; // ¡Ÿ¨O¶]¨∞ null window ≠Ï•ª≥]¶® +1 ™∫√ˆ´Y
		}
		// m in [alpha, beta]
		if(m > alpha){
			// element ¶s¶bÆ…
			if(record.val != nullptr){
				// ¶p™G¨O flag ¨O exact, ®∫¿≥∏”≠n§@ºÀ
				if(*record.flag == 0 && (this->cutOffDepth - depth) == *record.depth){
					cerr << "What the hell?\n";
					cerr << "alpha: " << alpha << " ,beta: " << beta << "\n";
					cerr << "record.val: " << *record.val << " , now m: " << m << "\n";
					// TODO: ´·®”πÍ¥˙µo≤{, ≥o√‰¡Ÿ¨O∑|ºQ, ß⁄ƒ±±o•iØ‡¨O¶]¨∞ lower bound 
					// ∏Ú upper bound flag ¶≥æ˜≤vß‚ alpha beta bound ±¿∞™(≥o¨q function ™∫≥Ã´e≠±), •H¶‹©Û¶^∂«™∫≠»¶≥•iØ‡∏Ú•H´e§£¶P
					// P.S. (≥o•u¨Oß⁄™∫±¿¥˙, ¶p™G¶≥±jéÕ™æπD¨∞§∞ªÚ¶A≥¬∑–ßi∂Dß⁄§F)
					// ´·®”¶AπÍ¥˙µo≤{°A®‰πÍ≠nº≤®Ï≥o≠” bug ª·√¯™∫, ƒY≠´√h∫√≠Ï¶]•iØ‡¨O¶]¨∞ s[][], color[] •iØ‡¶≥º≤®Ï hash ≠»
					// ´·®”¶Aµo≤{°A¶p™G TTB ¶^∂«¨O fail-soft, ¶nπ≥¡Ÿ¨O∑|¶≥≥o≠”∞›√D°A∂}©l¶b∑Q∑|§£∑|¨O¶]¨∞ alpha, beta ≥Q TTB cacheΩ’πL§ß´·≥y¶®§£¶P™∫ behavior
					// ´·®”§S¶A´‰¶“°A∑|§£∑|¨O¶]¨∞¥`¿ÙΩL≠±©“≥y¶®™∫ ---> µ≤Ω◊¨O°G¿≥∏”¨O§F
					assert(m == *record.val); // ¶p™G≤`´◊§@ºÀ, ±o®Ï™∫≠»•≤∂∑§@ºÀ
					// ®œ•Œ≥Ã∑s™∫≠»!!!!!
					*record.val = m;
				}
				// ¿À¨d≤`´◊¶≥®S¶≥≈‹≤`
