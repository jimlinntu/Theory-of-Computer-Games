...
public:
	...
	MOV Play(const BOARD &B, int remain_milliseconds){
		...
		assert(B.who != -1); // À³¸Ó·|³Q special condition ¾×±¼
		// Pre-evaluation
		...
		// ¦pªG¦]¬°¨S¦³¨B©Î¬O negaScout ·j¤£¥X¨B¨Ó
		if(BestMove.st == -1 || BestMove.ed == -1){
			// ¤@©w·|¦³¤l¥i¥HÂ½, ¦]¬° negaScout ¤@©w¦³ movList
			assert(randomFlip(B, BestMove));
		}
		...
		// ¦pªG¬O¨«¤l´N­nÀË¬d·|¤£·|­«½Æ
			...
			// ¦pªG flag ¤w¸g¬O 2 ¤F, ¨º´N¶ÃÂ½©Î¬O¶Ã¨«
			if(record.flag != nullptr && (*record.flag) >= 2){
				...
				if(B.sumCnt > 0) {
					...
					this->randomMove(nextB, BestMove, prohibitMove); // ¶Ã¨«, ¦ı¤£¯à¨«¨ì prohibit move
